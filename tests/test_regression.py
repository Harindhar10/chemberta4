...
class DummyRegressionBackbone(nn.Module):
...
class DummyCausalLM(nn.Module):
    ...
    def generate(
        ...
        """Append dummy token IDs so the generated output slice is non-empty."""
...
class TestRegressionHead:
    ...
    B, S = 3, 8  # batch size, sequence length
    ...
    def test_forward_shape_and_loss(self) -> None:
        ...
        stable training.
        """
        head = RegressionHead(DummyRegressionBackbone())
    ...
    def test_perfect_prediction_gives_epsilon_loss(self) -> None:
...
class TestCausalLMRegressionHead:
    ...
    def test_generate_and_parse_extracts_float(self) -> None:
